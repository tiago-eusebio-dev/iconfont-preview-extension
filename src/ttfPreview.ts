...
export class TTFEditorProvider implements vscode.CustomReadonlyEditorProvider<TTFDocument>{
  ...
  resolveCustomEditor(document: TTFDocument, webviewPanel: vscode.WebviewPanel, token: vscode.CancellationToken): void | Thenable<void> {
    ...
    const glyphUnicodes: string[] = []
    let glyphs: openType.GlyphSet
    try {
      buffer = this.nodeBufferToArrayBuffer(document.documentData)
      glyphs = openType.parse(buffer.buffer).glyphs
      for (let i = 0; i < glyphs.length; i++) {
        const glyph = glyphs.get(i)
        if (glyph.unicode) {
          glyphUnicodes.push(`&#x${glyph.unicode.toString(16)};`)
        }
      }
      console.log(openType.parse(buffer.buffer))
    } catch (e) {
      console.error(e)
    }

    const fontPath = document.uri.toString().split('/');
    const fontName = fontPath[fontPath.length - 1];

    const html = this.getHtmlForWebView(fontName, webviewPanel.webview, Buffer.from(buffer!!.buffer).toString('base64'));
    webviewPanel.webview.postMessage({ data: document.documentData, glyphs: glyphUnicodes })
    webviewPanel.webview.html = html
  }

  getHtmlForWebView(fontName: string, webview: vscode.Webview, data: String) {
